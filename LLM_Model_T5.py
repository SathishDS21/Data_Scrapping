...
def tokenize_function(tokenizer, texts, labels):
    ...
    # Convert the labels into string format suitable for T5
    tokenized_data['labels'] = tokenizer(
        [str(label) for label in labels],
        padding=True,
        truncation=True,
        max_length=2,  # Max length for label tokens
        return_tensors="pt"
    )['input_ids']
...
def train_model(train_dataset, val_dataset, tokenizer, label_encoder, class_weights, model_name='t5-small'):
    # Load T5 model for conditional generation (we can use this for sequence classification too)
