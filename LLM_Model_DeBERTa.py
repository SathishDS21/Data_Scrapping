...
def tokenize_function(tokenizer, texts, labels):
    logging.info("Tokenizing text data using DeBERTa tokenizer.")
...
def train_model(train_dataset, val_dataset, tokenizer, label_encoder, class_weights, model_name='microsoft/deberta-v3-base'):
    model = DebertaV2ForSequenceClassification.from_pretrained(model_name, num_labels=len(label_encoder.classes_))
    model.to(device)
